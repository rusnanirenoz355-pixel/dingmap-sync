...
006-G 不继续扩大为新功能包，先收敛 P0：

* 撤销产品级“人工辅助定位”：Dashboard 不展示入口，API 不接受 `manualAssist`，状态机不暴露 `manual_assist`。
* 失败 / blocked / 浏览器关闭后提供“重置上传任务”，重置后可重新自动上传。
* `Target page, context or browser has been closed` 等 Playwright 底层错误转为中文可恢复提示。
* Clean Table / 预览 / Excel 导入 / 钉图模板导出保持字段语义一致，联系人与电话合并展示为“联系人 电话”。
* 钉图模板仍保持固定 7 列：标记名称、详细地址、经度、纬度、备注、字段一、字段二。
* 无经纬度但有站点地址仍视为正常。
