...
## 2026-06-14：Task 007-C 人工验收修复与下班同步

### 已完成

* 优招钉图导出备注移除“岗位名称”，仅保留“薪资方案”和“新人政策”分段。
* `jobTitle` 继续保留在 Clean Table / raw 数据中，但不再写入优招钉图导出备注列。
* Dashboard 首页 Clean Table 接入独立表格滚动框，支持横向和纵向内部滚动，表头 sticky，页面不因 Clean Table 行数增多而无限拉长。
* 新增本地优招导出城市列表接口，导出城市下拉从本地已入库 `youzhao / web` 有效数据中读取城市，不触发优招网络采集。
* `/data-management` 不再展示或编辑面试时间。
* 已导入数据管理异常规则统一为服务端实时判断：站点名称为空或地址为空即异常；缺经纬度、电话、薪资、福利、岗位、备注、面试时间不再判异常。
* 当前本地管理统计：有效 770，异常 0，已删除 9。

### Task 007-D 待办记录

* 对账优招页面杭州数量与当前系统数量差异。
* 复核并修正后续业务线到目标图层映射。
* `/data-management` 后续增加多选软删除和删除当前筛选结果。
* 本轮仅记录待办，不实现 Task 007-D，不重新采集，不执行钉图上传。

### 验证

* 定向测试通过：6 个测试文件，39 项测试。
* `corepack pnpm verify` 通过：check、lint、27 个测试文件、144 项测试。
* 敏感文件检查无已跟踪 DB、导出 Excel、browser profile、截图、HAR、`.env` 或 `.auth`。

## 2026-06-15：Task 007 文档进度补录与跨电脑同步

### Task 007-C 完成提交

```text
commit:
7d3dd680780fe4b74481d237d4c3937cb6eb2a04

message:
fix: refine youzhao export and data management
```

### Task 007-C 已完成修复

* 优招钉图 Excel 的“备注”已删除岗位名称。
* 优招备注现在只包含薪资方案和新人政策。
* 字段一仍为站长姓名 + 空格 + 站长电话。
* 字段二仍为结算规则。
* 不影响 `manual_paste` 和普通 Excel 导出。
* Dashboard 首页 Clean Table 已增加独立纵向和横向滚动。
* 表头已 sticky。
* 页面 body 不再因宽表出现整体横向溢出。
* 城市下拉已改为本地真实城市列表，当前显示全部城市和杭州。
* 已删除“当前城市”选项。
* 城市列表只读取本地数据，不触发优招网络采集。
* `/data-management` 已移除面试时间的页面展示、编辑输入和保存提交。
* 数据库字段未删除，未做 schema migration。
* 异常规则已统一为：名称为空则异常，地址为空则异常，名称和地址均存在则正常。
...
### Task 007-D 待办：管理数据批量软删除

`/data-management` 后续增加每行复选框、多选、选择当前页、删除选中项和删除当前筛选结果。

删除必须使用现有软删除机制。“一键删除”定义为删除当前筛选条件下的全部记录；无筛选时才代表当前管理范围中的全部记录。

二次确认必须显示预计删除数量、当前筛选范围、数据来源范围和软删除说明。软删除后必须刷新管理列表、总数、正常/异常统计、Dashboard Clean Table、城市下拉、导出数量和图层统计。不得物理删除数据库记录或 `raw_records`。
